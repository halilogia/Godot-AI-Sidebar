...
**İlerleme:** 2349 → 1415 satır (1.1–1.8). 1.7'de kalıcı oturum durumu UI'sız `ChatSessionStore`'a taşındı; UI orkestrasyonu (akış temizleme, rozet, history paneli) bilinçli olarak ChatDock'ta kaldı. **Hedef:** ChatDock ≤ ~500 satır; yalnızca sahne bağlantısı + birimlerin kompozisyonu.
...
10. ✅ **Düzeltildi — İki ayrı "düşünme" göstergesi:** her LLM turunda akışa "Düşünülüyor (Ns)..." yazan yer tutucu asistan balonu ekleniyordu. Modelin gerçek thinking kartı sonra geldiği için balonun altına düşüyor (düşünce cevabın altında görünüyordu); metinsiz tool turlarında balon akışta asılı kalıyordu. Baz commit `1af07d3`'te de vardı. Yer tutucu kaldırıldı; bekleme süresi yalnızca durum rozetinde (`Waiting... (Ns)`). Testler: `ReasoningUITests` T11/T12.
