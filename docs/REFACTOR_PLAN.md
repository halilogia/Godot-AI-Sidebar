...
13. **Açık (Faz 2.1'de bulundu) — `file_time` iki dosya aracını saymıyor:** `replace_file_content` ve `delete_file` `file_ops` sayacına ekleniyor ama süreleri `file_time`'a eklenmiyor (kategori süresi listesi eksik; baz `1af07d3`'te de aynı). Telemetri kartındaki / export'taki dosya süresi eksik görünüyor.
