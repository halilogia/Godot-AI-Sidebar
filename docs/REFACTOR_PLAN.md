...
14. **Açık (Faz 2.4'te bulundu, ürün kararı) — Runner'ın Stop'u oyunu her durumda kapatıyor:** `AgentRunner.stop()` → `runtime_debugger.stop()` → `EditorInterface.stop_playing_scene()`. Oyunu ajan başlatmamış olsa bile (kullanıcı F5 ile açtıysa ya da başka bir runner başlattıysa) ajan durdurulunca oyun kapanır. İzleme durumu (`_is_monitoring`, log offset'leri, `_last_observation`) statiktir ve tüm runner'lar arasında ortaktır. Tek runner'da "Stop her şeyi durdurur" olarak kasıtlı olabilir; alt ajanlarda (ürün Faz 10) bir alt ajanın Stop'u ana ajanın oyununu kapatır. Headless'ta `EDITOR_REQUIRED` ile erken döndüğü için test edilemedi. Karar: Stop yalnızca bu runner'ın başlattığı oyunu mu durdursun, yoksa bugünkü davranış korunup alt ajanlara runtime araçları hiç mi verilmesin (ROADMAP Faz 10 zaten salt okuma ile başlıyor)?
