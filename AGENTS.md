...
5. **Dış ajan köprüsü arka kapı değildir:** `core/bridge/` üzerinden gelen her MCP çağrısı `ToolManager` → PermissionPolicy → PathPolicy → doğrulama hattından geçer; `EditorInterface`'e doğrudan erişmez. Dışarı açılan araçlar `AISidebarMcpProtocol.EXPOSED_TOOLS` (okuma / gözlem) ve `MUTATION_TOOLS` (açık sahnede Undo'lu küçük değişiklikler: `expected_scene_path` ve tek aktif yazıcı kilidi zorunlu; izin köprünün `/mcp on` ile açılmasıdır) izin listelerindedir; listeye eklenen her araç `tests/test_mcp_bridge.gd` ile ve güvenlik etkisi (değiştirici mi, onay gerekir mi) düşünülerek eklenir. Köprü yalnız `127.0.0.1`'de, token ile çalışır; token sohbete / loglara yazılmaz. **Dosya-öncelikli üretim:** dış ajan GDScript, `.tscn`, `.tres`, `.gdshader` ve veri dosyalarını kendi dosya araçlarıyla üretir, sonra `sync_project` çağırır; köprünün sahne araçları yalnız açık sahnede küçük, cerrahi, editör durumuna bağlı ve Undo/Redo gerektiren işler içindir. Köprü yüzeyi normal kod / dosya üretimini kopyalayan üst düzey araçlarla (`create_enemy`, `create_strategy_map` vb.) büyütülmez; bu kural `initialize` talimatında (`AISidebarMcpProtocol.INSTRUCTIONS`) dış ajana da verilir. **Tek metodoloji:** Godot geliştirme yöntemi (özellik geliştirme, sahne yazımı, hata ayıklama, runtime doğrulama…) ajandan bağımsızdır ve `integrations/claude-code/skills/` altında yazılıdır; Claude Code'a özgü olan yalnız paketleme ve her skill'in "Agent mapping" bölümüdür. Sidebar ajanının sistem istemi (`core/config/api_config.gd`) ve köprü talimatı aynı kurallarla uyumlu tutulur: bir kural değişince üçü aynı işte güncellenir. **Yetenek sınırı:** eklentiye yalnız Godot'a özgü yetenek eklenir; terminal, tarayıcı, genel indirme, paket yönetimi ve orkestrasyon dış ajanın (Claude Code vb.) ya da ileride Companion'ın işidir (bkz. `ROADMAP.md` → v3.x Çalışma modları).
...
| Godot geliştirme metodolojisi / ajan davranış kuralı | `integrations/claude-code/skills/`, sidebar sistem istemi (`core/config/api_config.gd`), köprü talimatı (`AISidebarMcpProtocol.INSTRUCTIONS`) birlikte |
