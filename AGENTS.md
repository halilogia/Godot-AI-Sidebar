...
5. **Dış ajan köprüsü arka kapı değildir:** `core/bridge/` üzerinden gelen her MCP çağrısı `ToolManager` → PermissionPolicy → PathPolicy → doğrulama hattından geçer; `EditorInterface`'e doğrudan erişmez. Dışarı açılan araçlar `AISidebarMcpProtocol.EXPOSED_TOOLS` (okuma / gözlem) ve `MUTATION_TOOLS` (açık sahnede Undo'lu küçük değişiklikler: `expected_scene_path` ve tek aktif yazıcı kilidi zorunlu; izin köprünün `/mcp on` ile açılmasıdır) izin listelerindedir; listeye eklenen her araç `tests/test_mcp_bridge.gd` ile ve güvenlik etkisi (değiştirici mi, onay gerekir mi) düşünülerek eklenir. Köprü yalnız `127.0.0.1`'de, token ile çalışır; token sohbete / loglara yazılmaz. **Dosya-öncelikli üretim:** dış ajan GDScript, `.tscn`, `.tres`, `.gdshader` ve veri dosyalarını kendi dosya araçlarıyla üretir, sonra `sync_project` çağırır; köprünün sahne araçları yalnız açık sahnede küçük, cerrahi, editör durumuna bağlı ve Undo/Redo gerektiren işler içindir. Köprü yüzeyi normal kod / dosya üretimini kopyalayan üst düzey araçlarla (`create_enemy`, `create_strategy_map` vb.) büyütülmez; bu kural `initialize` talimatında (`AISidebarMcpProtocol.INSTRUCTIONS`) dış ajana da verilir. **Tek metodoloji:** Godot geliştirme yöntemi (dosya-öncelikli üretim, sync, doğrula, çalıştır, kanıt topla, onar) ajandan bağımsızdır. Üç yerde yazılıdır: sidebar ajanının sistem istemi (`core/config/api_config.gd`), köprü talimatı (`AISidebarMcpProtocol.INSTRUCTIONS`) ve yerleşik skill'ler (`addons/godot_sidebar_ai/skills/`, Agent Skills standardı); bir kural değişince üçü aynı işte güncellenir. Skill'ler ve proje kuralları eklentinin kendi özelliğidir (`core/skills/`), açık standartlara uyar (`SKILL.md`, oyun projesinin kök `AGENTS.md`'si); MCP'ye ya da tek bir dış istemciye özgü skill / belge biçimi tutulmaz. **Yetenek sınırı:** eklentiye yalnız Godot'a özgü yetenek eklenir; terminal, tarayıcı, genel indirme, paket yönetimi ve orkestrasyon dış ajanın (Claude Code vb.) ya da ileride Companion'ın işidir (bkz. `ROADMAP.md` → v3.x Çalışma modları).
...
7. **Context Compaction:**
   * Çok adımlı görevlerde 2 adımdan eski araç çıktıları `AISidebarContextCompactor` ile 1-2 satırlık özetlere dönüştürülür; son 2 araç (`keep_recent_tools: 2`) tam korunur. `activate_skill` sonuçları (skill talimatları) hiç özetlenmez.
8. **Önce arayüz; Ayarlar güncel kalır (UI ve altyapı olarak):**
