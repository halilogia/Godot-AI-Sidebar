...
- [x] **v3.0.1 dış ajan mutasyon çekirdeği (kod; Ctrl+Z / WRITER_BUSY E2E final doğrulamada):** `add_node`, `set_node_property`, `instantiate_scene`, `attach_script_to_node`, `save_scene` dış ajana açık; ayrı yazma modu `/mcp write off | auto` (varsayılan `off`, kalıcı değil; sidebar'ın MANUAL / AUTO / FULL_AUTO modu dış ajana geçmez), köprüye özgü `expected_scene_path` koruması, tek aktif yazıcı (`WRITER_BUSY`, dış ajan için 60 sn kira), Undo/Redo. Kod tamam; kapanış koşulu gerçek Claude Code E2E: `add_node → set_node_property → save_scene → play_game → take_runtime_screenshot`, Ctrl+Z ve `WRITER_BUSY`.
- [x] **v3.0.2 etkileşimli onay (kod; onay E2E ve zaman aşımı ölçümü final doğrulamada):** `/mcp write ask`, dock'ta onay kartı (onayla / reddet), istemci koparsa, süre dolarsa ve köprü kapanırsa iptal. Süre geçici olarak 120 sn; Claude Code'un uzun MCP araç çağrısındaki gerçek zaman aşımı ölçülüp ayarlanacak. Onay beklenirken yazıcı kilidi tutulmaz: izin listesi → yazma modu → sahne ön kontrolü → onay → kilit → sahne tekrar kontrolü → ToolManager.
- [x] **Dosya-öncelikli dış ajan politikası (kod; açık `.tscn` yeniden yükleme senaryosu final doğrulamada):** "Üretim dosya-öncelikli, editör etkileşimi araç-öncelikli" kuralı `initialize` talimatında ve sahne araçlarının açıklamalarında; `sync_project(changed_files)` editörde açık olan yazılmış `.tscn`'i sidebar'ın `refresh_open_scenes()` mekanizmasıyla diskten yeniden yükler. Kural `AGENTS.md` §3.5'te.
- [x] **v3.1 Godot geliştirme skill'leri (ilk set; gerçek kullanımda ayarlama final doğrulamada):** `integrations/claude-code/skills/` altında yedi skill: proje başlatma, özellik geliştirme, sahne yazımı, hata ayıklama ve onarım, runtime doğrulama, refactor, milestone / sürüm akışı. Yeni API eklemezler; köprünün mevcut araçlarını dosya-öncelikli sırayla kullanmayı öğretirler. Kurulum: `integrations/claude-code/README.md`.
- [x] **v3.1 proje hafızası standardı:** oyun reposunda beş belge (`GAME_SPEC.md`, `ARCHITECTURE.md`, `ROADMAP.md`, `DECISIONS.md`, `KNOWN_ISSUES.md`); şablonlar `godot-project-bootstrap` skill'inin içinde, hangi skill'in hangi belgeyi ne zaman okuyup yazdığı `integrations/claude-code/README.md`'de. Özel veritabanı / görev grafiği yok.
- [x] **v3.2 otonom proje akışı ve benchmark altyapısı:** `godot-milestone-loop` skill'i (araştır → planla → uygula → sync → çalıştır → gözle → onar → doğrula → commit → sonraki madde; durma koşulları) ve `benchmarks/grand-strategy-slice/` (tek istem, kanıt türüyle kabul kriterleri, koşu başına kayıt alanları, boş proje kuran script).
- [ ] **v3 final stabilizasyon / doğrulama fazı:** v3.0.1–v3.2 hafif kontrollerle yazıldı (2026-09-26 stratejisi); ağır doğrulama burada topluca yapılır: tam `verify.ps1` + CI + regresyon, mutasyon testleri, GUI duman testi, Claude Code MCP E2E (okuma, mutasyon, `ask` onayı, Ctrl+Z, `WRITER_BUSY`, bağlantı kopması / zaman aşımı ve gerçek Claude Code araç çağrısı zaman aşımının ölçülmesi), dosya-öncelikli `.tscn` yazımında açık sahne yeniden yükleme / bayat editör senaryosu, runtime hataları, ekran görüntüleri, skill'lerin gerçek kullanımda ayarı, benchmark koşusu ve bulunan hataların düzeltmesi. Bilinen bekleyen düzeltme: sidebar tarafındaki `WRITER_BUSY` mesajı modele "tekrar dene" diyor; sidebar modeli bekleyemediği için tekrar denedi ve başarı uydurdu (tamamlanma kapısı görevi Failed işaretledi).
...
- [ ] **Sonra (ihtiyaç ölçülünce):** Claude Code plugin paketi (MCP + skills), diğer MCP istemcileri (Cursor, Codex, Antigravity: köprü standart Streamable HTTP + Bearer başlığı konuşur; istemci başına bağlantı ayarı ancak o istemciyle denenince belgelenir), Companion yalnız benchmark / gerçek kullanımda ölçülmüş eksik varsa, alt ajanlar (Faz 10) en son.
