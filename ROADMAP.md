...
- [x] **v3.0.1 dış ajan mutasyon çekirdeği (kod; Ctrl+Z / WRITER_BUSY E2E final doğrulamada):** `add_node`, `set_node_property`, `instantiate_scene`, `attach_script_to_node`, `save_scene` dış ajana açık (izin: `/mcp on`; sidebar'ın MANUAL / AUTO / FULL_AUTO modu dış ajana geçmez), köprüye özgü `expected_scene_path` koruması, tek aktif yazıcı (`WRITER_BUSY`, dış ajan için 60 sn kira), Undo/Redo. Kod tamam; kapanış koşulu gerçek Claude Code E2E: `add_node → set_node_property → save_scene → play_game → take_runtime_screenshot`, Ctrl+Z ve `WRITER_BUSY`.
- [x] ~~**v3.0.2 etkileşimli onay**~~ **(kaldırıldı, 2026-09-26):** `/mcp write off | ask | auto` ve dock onay kartı yazıldı, benchmark sonrası kaldırıldı. Benchmark'ta ajan sahne araçlarını hiç kullanmadı (dosya-öncelikli akış); mod yalnız Ctrl+Z ile geri alınabilen küçük düzenlemeleri denetliyordu ve Claude Code'un kendi onayıyla çift onay yaratıyordu. İzin `/mcp on`; onay dış ajanın izin sisteminde.
