	...
	return [
		...
		},
		{
			...
			"function": {
				...
				"description": "Bir GDScript dosyasını oluşturur veya içeriğini günceller ve Godot editörüne canlı olarak derletir.",
				"parameters": {
					"type": "object",
					"properties": {
						"file_path": { "type": "string", "description": "Dosya yolu (örn: res://scripts/Player.gd)." },
...
static func _create_or_update_script(args: Dictionary) -> Dictionary:
	...
	return AISidebarToolResult.ok({"file_path": path}, "Script başarıyla kaydedildi ve editörde güncellendi.")
...
static func _validate_script(args: Dictionary) -> Dictionary:
	...
	if not script_res:
		return AISidebarToolResult.err("SYNTAX_ERROR", "Script derlenemedi, sözdizimi (syntax) hatası var.", true)
		
	return AISidebarToolResult.ok({"file_path": path, "valid": true}, "Script başarıyla doğrulandı.")
