...
## Connection: close başlığı ve hızlı I/O ile keep-alive gecikmelerini sıfırlar.
...
func get_request(url: String, headers: PackedStringArray) -> Error:
	...
	return _models_http.request(url, final_headers, HTTPClient.METHOD_GET)
...
func post_request(url: String, headers: PackedStringArray, body_json: String) -> Error:
	...
	if not _has_header(final_headers, "Connection"):
		final_headers.append("Connection: close")
	return _chat_http.request(url, final_headers, HTTPClient.METHOD_POST, body_json)
