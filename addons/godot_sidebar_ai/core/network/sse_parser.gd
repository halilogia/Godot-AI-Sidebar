...
static func parse_response(raw_text: String) -> Dictionary:
	...
	if not is_sse:
		...
		if json_res is Dictionary:
			...
			elif json_res.has("error"):
				var err_msg = json_res["error"].get("message", "Bilinmeyen API hatası")
	...
	return {
		"content": total_content.strip_edges(),
		"thinking": total_thinking.strip_edges(),
