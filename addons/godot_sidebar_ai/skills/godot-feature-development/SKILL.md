...
4. **Sync and validate:** read the tool schema first. Call `sync_project` with every changed path in `changed_files`, then `validate_script` with the required `file_path` for changed scripts. This checks in-memory compilation in the current editor context; it does not prove clean-cache dependency compilation or runtime behavior. Follow the project's `AGENTS.md` / development guide and run its documented headless checks before claiming compilation or tests pass.
