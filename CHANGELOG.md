...
* **Dış ajan değişikliklerini tek tek onaylama (v3.0.2):** `/mcp write ask` ile dış ajanın her sahne değişikliği için sidebar'da bir onay kartı açılır; Onayla deyince uygulanır, Reddet deyince ajan "kullanıcı reddetti" sonucunu alır. Karar verilmezse (şimdilik 120 sn), ajanın bağlantısı koparsa ya da köprü kapatılırsa değişiklik yapılmaz ve kart kapanır.
...
* **Claude Code skill'leri (v3.1):** `integrations/claude-code/skills/` altında yedi skill (proje başlatma, özellik geliştirme, sahne yazımı, hata ayıklama, runtime doğrulama, refactor, milestone / sürüm akışı). Oyun projesinde `.claude/skills/` içine kopyalanınca Claude Code köprüyü dosya-öncelikli ve kanıtla doğrulayan bir akışla kullanır; kurulum `integrations/claude-code/README.md`.
* **Otonom milestone akışı ve grand strateji benchmark'ı (v3.2 altyapı):** `godot-milestone-loop` skill'i Claude Code'un yol haritasındaki maddeleri tek tek uygulayıp kanıtla doğrulayarak ilerlemesini ve ne zaman durup soracağını tanımlar. `benchmarks/grand-strategy-slice/` tek istemlik bir grand strateji dikey kesiti, kabul kriterleri ve boş benchmark projesi kuran bir script içerir.
