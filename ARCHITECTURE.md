...
* **Dış ajan sahne mutasyonları (v3.0.1 / v3.0.2):** Sunucu her mutasyonu şu sırayla geçirir: izin listesi → yazma modu (`/mcp write off | ask | auto`; varsayılan `off`, kalıcı değil: editör açılışında ve `/mcp off`'ta kapanır) → köprüye özgü zorunlu `expected_scene_path` ön kontrolü (editörde açık sahne değilse `ACTIVE_SCENE_NOT_CONFIRMED`; ToolManager'a gitmeden args'tan çıkarılır, `instantiate_scene`'in kaynak `scene_path`'iyle karışmaz) → `ask` modunda kullanıcı onayı (beklerken yazıcı kilidi tutulmaz) → yazıcı kilidi (`writer_lock.gd`; sidebar ajanı yazıyorsa `WRITER_BUSY`) → etkin sahne tekrar kontrolü (değiştiyse yeni alınan kilit bırakılır) → ToolManager → MutationService / Undo-Redo. Son üç adım senkron: kontrol ile yürütme arasında editör karesi geçmez.
* **[`external_approvals.gd`](addons/godot_sidebar_ai/core/bridge/external_approvals.gd):** `ask` modunun bekleyen onay kaydı. Köprü `request()` ile kayıt açar ve kare kare bekler; sonuç kullanıcı kararı (`approved` / `denied`) ya da köprünün iptali (`timeout`, `disconnected` — istemci bağlantısı düştü, `bridge_stopped` — `/mcp off`, `/mcp write off`, eklenti kapanışı). Arayüzü tanımaz; `requested` / `resolved` sinyalleriyle konuşur. Onay süresi (`APPROVAL_TIMEOUT_MSEC`, 120 sn) geçicidir: Claude Code'un gerçek araç çağrısı zaman aşımı final doğrulamada ölçülecek.
* **[`external_approval_presenter.gd`](addons/godot_sidebar_ai/ui/presenters/external_approval_presenter.gd):** Onay kaydının her isteği için dock'ta `ApprovalCard` açar, kararı kayda iletir, iptalde kartı kapatır. `plugin.gd` köprüyü kurduktan sonra `ChatDock.bind_external_approvals()` ile bağlar. Transcript'e yazılmaz (bilinçli, §7): bu istekler bir sidebar görevinin parçası değildir; iz dış ajanın araç sonucunda (`USER_DENIED`, `APPROVAL_TIMEOUT` …) ve editör konsolundadır.
